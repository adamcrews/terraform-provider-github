...
func mustCreateTestOrganizationAgentsSecret(t *testing.T) string {
	...
	if _, err := testAccConf.meta.v3client.Agents.CreateOrUpdateOrgSecret(t.Context(), testAccConf.meta.name, secretName, github.OrgSecretRequest{
		Visibility:     "all",
		KeyID:          publicKey.GetKeyID(),
		EncryptedValue: encryptedValue,
	}); err != nil {
...
func mustUpdateOrganizationAgentsSecret(t *testing.T, name, value string) {
	...
	if _, err := testAccConf.meta.v3client.Agents.CreateOrUpdateOrgSecret(t.Context(), testAccConf.meta.name, name, github.OrgSecretRequest{
		Visibility:     "all",
		KeyID:          publicKey.GetKeyID(),
		EncryptedValue: encryptedValue,
	}); err != nil {
